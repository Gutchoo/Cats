...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_list = [i for i in paragraphs if select(i) == True]
    if k >= len(new_list):
        return ''
    else:
        return new_list[k]
...
def about(topic):
    ...
    def helper(paragraph):
        lower_case = lower(paragraph)
        new_par = remove_punctuation(lower_case)
        split_par = split(new_par)
        result = []
        for i in topic:
            result += [i in split_par] 
...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words == []:
        ...
        for i in range(len(reference_words)):
            if typed_words[:len(reference_words)][i] == reference_words[i]:
                counter += 1
        return counter/len(typed_words) * 100
    else:
        counter = 0
        for i in range(len(typed_words)):
            if typed_words[i] == reference_words[:len(typed_words)][i]:
                counter += 1
        return counter/len(typed_words) * 100
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == "":
        ...
        if limit < 0:
            ...
            substitution = minimum_mewtations(goal[0]+ start[1:], goal, limit -1) + 1
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        times += [helper(times_per_player[i])]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    times_list = []
    players_list = [[] for b in player_indices]

    for a in word_indices:
        for i in player_indices:
            times_list += [time(match, i, a)]
        min_index = times_list.index(min(times_list))
        times_list = []
        players_list[min_index] = players_list[min_index] + [get_words(match)[a]]
